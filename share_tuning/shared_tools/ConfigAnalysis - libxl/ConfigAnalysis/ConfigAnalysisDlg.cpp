...
BOOL CConfigAnalysisDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	ReadPath();
	m_pProgressBar = new CProgressBarDlg();
	m_pProgressBar->Create(IDD_DIALOG_PROGRESS);
...
void CConfigAnalysisDlg::OnBnClickedButton4()
{
	...
	_msg.AnalysisDesign(book,m_pProgressBar);
...
//	CDialogEx::OnOK();
	...
	if (!fileDir.IsEmpty())
	{
		strFileName.Format("%s\\",fileDir);
	}
	CFileDialog fileDlg(TRUE, NULL,strFileName);
...
void CConfigAnalysisDlg::OnBnClickedButton3()
{
	...
	Design _msg;
	_msg.AnalysisOneConfig(book, _configName);
	_msg.AnalysisOneConfigExcel(book, _configName, m_pProgressBar);
...
void CConfigAnalysisDlg::OnBnClickedButton5()
{
	...
	m_pProgressBar->ShowWindow(SW_SHOW);
	_msg.AnalysisDesign(book,m_pProgressBar);
	_msg.AnalysisAllConfigExcel(book, m_listBox, m_pProgressBar);
	m_pProgressBar->ShowWindow(SW_HIDE);
	_msg.m_bIsAllExport = false;
	book->release();
