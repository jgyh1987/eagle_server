...
void ClientMsgCS::AnalysisClientMsg(libxl::Book* book)
{
	CString url;
    ...
    m_pProgress = CProgressBarDlg::Instance();
	bool bBeginProgress = true;
	CString strText;
	CString strValue;
	double iNum = 0;
	int iTotal = book->sheetCount() - 1;
	for (int number = 1; number < 20; ++number)
	{
		libxl::Sheet* sheet = book->getSheet(number);
		...
		if (bBeginProgress)
		{
			...
			m_pProgress->InitializationProgress(iTotal);
			bBeginProgress = false;
		}
		else
		{
			m_pProgress->StepProgress(number);
		}
...
void ClientMsgCPP::AnalysisClientMsg(libxl::Book* book)
{
	CString url;
...
void ClientMsgCPP::DealMsgItem()
{
	...
	{
		CString _compare = ClassProperty[l].type;
		if (_compare == "BYTE" || _compare == "uint32_t" || _compare == "uint64_t" || _compare == "double")
	...
	for (int l = 0; l < ClassProperty.size(); ++l)
	{
		...
		{
			ofilecpp << "		if (!writeStream->Write((double)this->" << ClassProperty[l].name << ")) return -1;" << endl;
		}
		...
		else if (_compare == "std::vector")
		{
			...
			if (_compareDetail == "std::string")
				...
				ofilecpp << "			if (!writeStream->Write((double)(*it))) return -1;" << endl;
			else
	...
	for (int l = 0; l < ClassProperty.size(); ++l)
	{
		...
		{
			ofilecpp << "		if (!readStream->Read((double&)this->" << ClassProperty[l].name << ")) return -1;" << endl;
		}
		else if (_compare == "std::string")
		{
		...
		else if (_compare == "std::vector")
		{
			...
			if (_compare == "BYTE")
				...
				ofilecpp << "				if (!readStream->Read((double&)__inner__)) return -1;" << endl;
			else if (_compare == "std::string")
			{
				ofilecpp<<"				const char* p;"<<endl;
				ofilecpp<<"				size_t len;"<<endl;
...
// 将一个double类型小数点后面的零去掉
std::string ClientMsg::valueToString(double value)
{
	char buffer[32];
...
double ClientMsg::Round(double value)
{
