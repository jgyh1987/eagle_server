#include "WGateService.h"
#include "ListenServer.h"
#include "WGateClientTask.h"
#include "Defs.h"
#include "WGateConnectClient.h"

CWGateServer *ServerImp()
{
    return (CWGateServer *)g_pServerImp;
}

CWGateServer::CWGateServer(const std::string &name)
    :CImpServer(SERVER_TYPE_MAX, name)
{
	m_needRemoveConnects.reserve(32);

}
    
CWGateServer::~CWGateServer()
{

}

void CWGateServer::Update()
{


    return;
}

void CWGateServer::UpdateSec()
{
	for (auto p : m_needRemoveConnects)
	{
		ServerImp()->RemoveConnectClient(p);
	}
	m_needRemoveConnects.clear();

    return;
}

void CWGateServer::Update5Sec()
{
    //Cmd::t_Server_Load_Notify notify;
    //notify.loadInfo = 0;
    //SendCmdToConnect(&notify, sizeof(notify), SERVER_TYPE_WMATCH_SERVER);


    if (!m_proxyServer.serverId)
    {
        Cmd::t_Server_Req_ProxyServer send;
        ServerImp()->SendCmdToConnect(&send, sizeof(send), SERVER_TYPE_CENTRAL_SERVER, 0);
    }

    return;
}

void CWGateServer::ProxySendCmd(void *cmd, uint32_t len, uint32_t serverType, uint32_t serverId)
{
	if (!m_proxyServer.serverId)
		return;

    Cmd::t_Server_Send_Trans_Msg* send = NULL;
    ServerCmdBuffer(send);
    memcpy(send->data, cmd, len);
    send->dataLen = len;
    send->targetServer.serverType = serverType;
    send->targetServer.serverId = serverId;
    send->srcServer.serverId = g_serverId;
    send->srcServer.serverType = SERVER_TYPE_WGATE_SERVER;

    SendCmdToConnect(send, send->Size(), SERVER_TYPE_PROXY_SERVER, 0);

    return;
}

void CWGateServer::ClearProxyServer()
{
	m_proxyServer.Clear();

	return;
}

