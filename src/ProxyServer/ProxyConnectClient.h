#ifndef _PROXY_CONNECTCLIENT_H_
#define _PROXY_CONNECTCLIENT_H_
...
class CProxyConnectClient : public CServerConnectClientEx
{
public:
    CProxyConnectClient(const std::string &name, uint32_t serverType, uint32_t serverId, const std::string &ip = "127.0.0.1",const unsigned short port = 18057, uint32_t queuesize = 102400)
...
template<class cmd>
unsigned char* CProxyConnectClient::ServerCmdBuffer(cmd* &name) {
    m_sendBuffer.ReSet();
